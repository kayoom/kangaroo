    ...
    describe Relation do
      ...
      describe '#[]' do
        it 'doesnt touch existing limit clause' do
          @target.should_receive(:execute_query).with hash_including(:limit => 2)
          @relation.limit(2)[1]
        end

        it 'doesnt touch existing offset clause' do
          @target.should_receive(:execute_query).with hash_including(:offset => 40)
          @relation.offset(40)[10]
        end

        it '[n] returns the n-th record' do
          @target.should_receive(:execute_query).with hash_including(:limit => 1, :offset => 21)
          @relation[21]
        end

        it '[n, m] return m records skipping n' do
          @target.should_receive(:execute_query).with hash_including(:limit => 4, :offset => 21)
          @relation[21, 4]
        end

        it '[n..m] returns records n to m' do
          @target.should_receive(:execute_query).with hash_including(:limit => 5, :offset => 21)
          @relation[21..25]
        end
      end
    end
